DashboardFolder <- RAWmisc::DashboardFolder
...
print(RAWmisc::DashboardFolder("data_raw"))
allRawDataFiles <- list.files(RAWmisc::DashboardFolder("data_raw"))
file <- max(allRawDataFiles)
print(file)
if(!is.na(file)){
  x <- readxl::read_excel(file.path(RAWmisc::DashboardFolder("data_raw"),file))
  temp <- lapply(x,class)
  classes <- c()
  ...
  x <- readxl::read_excel(file.path(RAWmisc::DashboardFolder("data_raw"),file),col_names=names(x),col_types=newClasses,skip=1)
  ...
  if(Sys.getenv("COMPUTER")=="smhb") unlink(file.path(RAWmisc::DashboardFolder("data_raw"),allRawDataFiles),force=TRUE)
